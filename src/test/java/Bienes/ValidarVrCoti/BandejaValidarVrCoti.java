...
public class BandejaValidarVrCoti {

	public void tareaValidadVRCoti(WebDriver driver, WebDriverWait wait, String cotizacion) throws InterruptedException{    
		...
		waitCuore.repited(driver, wait);
		wait.until(ExpectedConditions.elementToBeClickable(By.id("Validar_VR_Cotización")));
		waitCuore.repited(driver, wait);
		driver.findElement(By.xpath("//*[contains(text(),'Pendientes:')]")).click();
		driver.findElement(By.id("Validar_VR_Cotización")).click();
		Thread.sleep(1000);
		waitCuore.repited(driver, wait);
